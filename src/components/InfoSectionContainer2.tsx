import { FunctionComponent, memo, useMemo } from "react";
import CSS, { Property } from "csstype";

type InfoSectionContainer2Type = {
  chatTeamText?: string;
  teamChatText?: string;
  teamChatThreadText?: string;

  /** Style props */
  propTop?: Property.Top;
};

const InfoSectionContainer2: FunctionComponent<InfoSectionContainer2Type> =
  memo(({ chatTeamText, teamChatText, teamChatThreadText, propTop }) => {
    const infoSectionStyle: CSS.Properties = useMemo(() => {
      return {
        top: propTop,
      };
    }, [propTop]);

    return (
      <div
        className="absolute top-[103.5rem] left-[calc(50%_-_580px)] w-[72.5rem] flex flex-row items-center justify-between text-left text-[3.13rem] text-dark-blue font-paragraph-mobile"
        style={infoSectionStyle}
      >
        <img
          className="relative w-[35.31rem] h-[35.31rem]"
          alt=""
          src={chatTeamText}
        />
        <div className="w-[28.5rem] flex flex-col items-start justify-start gap-[3rem]">
          <div className="self-stretch flex flex-col items-start justify-start gap-[1.5rem]">
            <b className="self-stretch relative leading-[4rem]">
              {teamChatText}
            </b>
            <div className="self-stretch relative text-[1.13rem] leading-[1.69rem] text-gray-500">
              {teamChatThreadText}
            </div>
          </div>
          <div className="flex flex-row items-center justify-start gap-[0.5rem] text-[1rem] text-soft-blue">
            <div className="relative [text-decoration:underline] leading-[150%] font-medium">{`Learn more `}</div>
            <img
              className="relative w-[1.88rem] h-[1.88rem]"
              alt=""
              src="/iconactionarrow-right-alt-24px3.svg"
            />
          </div>
        </div>
      </div>
    );
  });

export default InfoSectionContainer2;
